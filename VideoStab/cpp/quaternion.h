...
public:
    ...
    inline Vector2<T> lerp (T fact, const Vector2<T>& vec) const {
        return (*this) + (vec - (*this)) * fact;
    }
...
public:
    ...
    inline Vector3<T> lerp (T fact, const Vector3<T>& vec) const {
        return (*this) + (vec - (*this)) * fact;
    }
...
public:
    ...
    inline Vector4<T> lerp (T fact, const Vector4<T>& vec) const {
        return (*this) + (vec - (*this)) * fact;
    }
...
*/
    ...
    Quaternion<T> conjugate (const Quaternion<T>& quat) const {
        return Quaternion<T>(-quat.v, quat.w);
    }

    Quaternion<T> inverse (const Quaternion<T>& quat) const {
        return conjugate(quat) * ( 1.0f / magnitude(quat));
    }
    
    Quaternion<T> lerp (T fact, const Quaternion<T>& quat) const {
        return Quaternion<T>(v.lerp(fact, quat.v), (1 - fact) * w + fact * quat.w);
    }
